"""Sanity checks for SwinGeoLinkSpatialBiasLandCoverGraphFusion."""

import torch
from mmseg.structures import SegDataSample

from mmgeo.models.swin_geolink_graph_fusion_v2 import SwinGeoLinkSpatialBiasLandCoverGraphFusion
...
def make_valid_graph(num_nodes: int = 6):
    ...
    centroid = polygon_pos.view(num_nodes, 4, 2)[:, -1, :]
    points = polygon_pos.view(num_nodes, 4, 2)
    bbox = torch.cat([points.min(dim=1).values, points.max(dim=1).values], dim=-1)
    return dict(
...
def main():
    ...
    module.train()
    out_train = module([feat.clone()], data_samples)
    aux_losses = module.get_aux_losses()
    assert out_train[0].shape == feat.shape
    assert aux_losses is not None
    assert 'loss_landcover_aux' in aux_losses
    assert torch.isfinite(aux_losses['loss_landcover_aux']).all()
    print('Swin GeoLink v2 spatial-bias graph fusion shape sanity check passed')
